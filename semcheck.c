        ...
        if(simple && node->category != CONSTANT_t) {
            if(node->scope == 0) {
                if(isAssignmentLHS == __TRUE) {
                    fprintf(pFile, "getstatic %s/%s ", program_name, node->name);
                    ...
                    fprintf(pFile, "putstatic %s/%s ", program_name, node->name, node->name);
...
struct expr_sem *verifyFuncInvoke( const char *id, struct expr_sem *exprList, struct SymTable *table, int scope )
{
    ...
    fprintf(pFile, "\tinvokestatic %s/%s(",program_name, node->name);
