...
int     label_num = 0;          //  count label 
int     labelLoopNum= 0;        // count Loop's label 
char* myname;
//project4 flags
int is_main = 0;
int globalVari_stmt = 0;
int simple = 0;              // where are  in simple_stmt
int assign_stmt = 0;              // where are  in assignment
int is_const = 0;
int print_stmt = 0;               // where are  in Print stmt
int read_stmt = 0;                // where are  in Read stmt
int param_stmt = 0;               // where are in decl function parameters
int condition_stmt = 0;           // where are in condition stmt
int return_stmt1 = 0;
int for_stmt1 = 0;                 // where are in for stmt
int while_stmt1 = 0;               // where are in while stmt
int my_read=0;
int my_procall=0;
%}
...
program_body: {globalVari_stmt = 1;} opt_decl_list {globalVari_stmt = 0;} opt_func_decl_list {is_main = 1;} compound_stmt 
			;
...
literal_const   : INT_CONST
			{
              ...
              if(assign_stmt || print_stmt || return_stmt1 || condition_stmt || for_stmt1 || while_stmt1) {
                  fprintf(pFile, "\tldc %d\n", tmp);
              }
			}
			| OP_SUB INT_CONST
...
param       :{parametter_num = 0;} id_list MK_COLON type 
            { 
                $$ = createParam( $2, $4 ); 
...
simple_stmt : var_ref 
			...
			{ 
			    ...
			    */
                ...
                if(simple && node->category != CONSTANT_t) {
                    if (node->scope > 0) {
                ...
                } else if (simple && node->category == CONSTANT_t) {
                    switch(node->type->type) {
                        case INTEGER_t:
            ...
            boolean_expr MK_SEMICOLON 
            {
                ...
                verifyScalarExpr( $3, "print" ); print_stmt = 0;
            }
 			| READ
            {
...
proc_call_stmt  : ID MK_LPAREN {my_procall=1;}opt_boolean_expr_list MK_RPAREN MK_SEMICOLON
			{
...
return_stmt : RETURN
              ...
              boolean_expr MK_SEMICOLON
			{
                ...
                case BOOLEAN_t:
                    ...
                    break;
                case STRING_t:
                    //fprintf(pFile, "\tslocal %d ; local variable number %d is %d\n" ,node->name, node->symLocalNum,node->name);
                    break;
                case REAL_t:
                    fprintf(pFile, "\tfreturn\n");
                    break;
...
boolean_expr: boolean_expr OP_OR boolean_term
			{ 
              //if(read_stmt) printf("is read [boolean op_or]\n");
			  verifyAndOrOp( $1, OR_t, $3 );
			  $$ = $1;
			}
			| boolean_term { $$ = $1;/* if(read_stmt) printf("is read [boolean]\n");*/}
			;
...
expr        : expr add_op{ assign_stmt=1;} term
			{
...
var_ref     : ID
			{
			    //if(read_stmt) printf("is read [var_ref]\n");

                $$ = createExprSem( $1 );
              
                struct SymNode *node = 0;               
                if(my_read){assign_stmt=0;  my_read=0;}
                if(for_stmt1) {
          ...
          if(assign_stmt || print_stmt || return_stmt1 || condition_stmt || for_stmt1 || while_stmt1) {
                ...
                if(read_stmt) {
                   // if(read_stmt) printf("is read [ID]\n");
